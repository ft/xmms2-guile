...
(define* (make-value-payload data #:key (tagged #t))
  (cond ((int64? data) (make-int64-payload data #:tagged tagged))
        ...
        ((association-list? data) (make-dictionary-payload data #:tagged tagged))
...
(define* (make-dictionary-payload data #:key (tagged #t))
  ...
  (let ((alist (or (and (dictionary? data)
                        (dictionary-data data))
                   data)))
    (let loop ((rest (reverse alist))
               (acc '()))
      (if (null? rest)
